...
function Reviews() {
  ...
  useEffect(() => {
    // Simulación: comprobamos si el usuario está autenticado (futuro JWT/localStorage)
    const auth = localStorage.getItem('authToken');
    setIsAuthenticated(!!auth);

    // Simulación: cargar reseñas destacadas desde backend (aquí simuladas)
  ...
  const handleSubmit = (e) => {
    ...
    const newReview = {
      ...formData,
      user: isAuthenticated ? 'Tú' : 'Anónimo', // Reemplazar con user.name desde backend
    };
  ...
  const handleAvatarClick = () => {
    if (isAuthenticated) {
      ...
      setNotification('¡Inicia sesión para dejar una reseña!');
      ...
      <header className="bg-gray-900 bg-opacity-90 text-gray-100 p-4 shadow-lg z-10">
        <div className="container mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <nav className="flex flex-col md:flex-row gap-2 md:gap-4 mb-2 md:mb-0">
            <Link to="/" className="px-4 py-2 bg-transparent border-2 border-indigo-800 rounded-lg hover:bg-indigo-800 transition-all text-base font-body font-semibold">Inicio</Link>
            <Link to="/reviews" className="px-4 py-2 bg-indigo-800 rounded-lg hover:bg-indigo-700 transition-all text-base font-body font-semibold">Reseñas</Link>
